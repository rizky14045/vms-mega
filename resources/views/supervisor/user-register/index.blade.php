        ...
        <div class="card">
            <div class="d-flex justify-content-between align-items-center px-3 pt-3">
                <div class="search-from">
                    <form action="{{route('supervisor.user-register.index')}}" method="GET">
                ...
                {{-- <a href="{{ route('supervisor.user-register.create') }}" class="btn btn-success">Tambah Data</a> --}}
            </div>         
            <div class="card-body">
                @if(session('error'))
                    ...
                    <table class="table table-bordered table-striped mb-3">
                        ...
                        <tbody>
                            ...
                            <tr>
                                ...
                                <td class="text-nowrap text-capitalize">{{ date('d-m-Y', strtotime($register->visit_date)) }}</td>
                                ...
                                <td class="text-nowrap">
                                    <a href="{{route('supervisor.user-register.detail',['id'=>$register->id])}}" class="btn btn-warning btn-sm">Detail</a>
                                    {{-- <a href="{{route('supervisor.user-register.reschedule',['id'=>$register->id])}}" class="btn btn-primary btn-sm">Reschedule</a> --}}
                                    @if ($register->rangking == 1) 
                                        <form action="{{route('supervisor.user-register.approve',['id'=>$register->id])}}" method="POST" class="d-inline delete-form" id="delete-form">
                                            @csrf
                                            @method('PATCH')
